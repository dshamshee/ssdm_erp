...
function createRandomUser() {
    return {
        ...
        role: faker.helpers.arrayElement(["superAdmin", "admin", "student"]) as "superAdmin" | "admin" | "student",
    };
}

async function main() {
    const users = faker.helpers.multiple(createRandomUser, { count: 5 });
    const insertedUsers = await db.insert(user).values(users).returning({ id: user.id });
