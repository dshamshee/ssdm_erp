...
const DEPARTMENTS = [
  {
    ...
    description:
      "Classical mechanics, quantum dynamics, and astrophysics labs.",
  },
  {
    code: "CHEM",
    name: "Dept of Chemistry",
    description:
      "Organic synthesis, physical chemistry, and material sciences.",
  },
  {
    code: "MATH",
    name: "Dept of Mathematics",
    description:
      "Pure logic, advanced calculus, linear algebra, and topological studies.",
  },
  {
    code: "COMP",
    name: "Dept of Computer Apps",
    description:
      "Full-stack development, software engineering, and database systems.",
  },
  {
    code: "COMM",
    name: "Dept of Commerce",
    description:
      "Corporate accounting, financial management, tax laws, and market auditing.",
  },
];
...
const SUBJECTS_POOL = [
  ...
  {
    code: "PHY-MJC2",
    name: "Electricity, Magnetism & Wave Optics",
    type: "MJC",
    hasPractical: true,
    fee: 600,
  },
  {
    code: "PHY-MIC1",
    name: "Introductory General Physics",
    type: "MIC",
    hasPractical: false,
    fee: 0,
  },
  {
    code: "CHM-MJC1",
    name: "Basic Organic & Inorganic Principles",
    type: "MJC",
    hasPractical: true,
    fee: 800,
  },
  ...
  {
    code: "CHM-MIC1",
    name: "General Chemistry Essentials",
    type: "MIC",
    hasPractical: false,
    fee: 0,
  },
  ...
  {
    code: "MAT-MJC2",
    name: "Real Analysis & Linear Algebra",
    type: "MJC",
    hasPractical: false,
    fee: 0,
  },
  {
    code: "MAT-MIC1",
    name: "Foundational Business Mathematics",
    type: "MIC",
    hasPractical: false,
    fee: 0,
  },
  {
    code: "BCA-MJC1",
    name: "Object-Oriented Programming via C++",
    type: "MJC",
    hasPractical: true,
    fee: 1000,
  },
  ...
  {
    code: "BCA-MIC1",
    name: "Fundamentals of IT & Office Tools",
    type: "MIC",
    hasPractical: false,
    fee: 0,
  },
  ...
  {
    code: "COM-MIC1",
    name: "General Principles of Microeconomics",
    type: "MIC",
    hasPractical: false,
    fee: 0,
  },
  {
    code: "ENG-MJC1",
    name: "Introduction to European Classical Literature",
    type: "MJC",
    hasPractical: false,
    fee: 0,
  },
  ...
  {
    code: "BUS-MDC1",
    name: "Entrepreneurship & Startup Dynamics",
    type: "MDC",
    hasPractical: false,
    fee: 0,
  },
  ...
  {
    code: "GEN-SEC1",
    name: "Advanced Web Styling & UI Design",
    type: "SEC",
    hasPractical: true,
    fee: 400,
  },
  ...
  {
    code: "GEN-VAC1",
    name: "Environmental Studies & Eco-Systems",
    type: "VAC",
    hasPractical: false,
    fee: 0,
  },
  {
    code: "GEN-VAC2",
    name: "Ethics, Culture & Human Values",
    type: "VAC",
    hasPractical: false,
    fee: 0,
  },
...
const COURSES_POOL = [
  {
    deptCode: "PHYS",
    code: "BSC-PHYS",
    name: "B.Sc (Honors) Physics",
    type: "UG Regular",
    duration: 4,
    description:
      "Four-year undergraduate physics framework focusing on mechanics and theory.",
  },
  {
    deptCode: "CHEM",
    code: "BSC-CHEM",
    name: "B.Sc (Honors) Chemistry",
    type: "UG Regular",
    duration: 4,
    description:
      "Rigorous organic, inorganic, and physical laboratory chemical program.",
  },
  {
    deptCode: "MATH",
    code: "BSC-MATH",
    name: "B.Sc (Honors) Mathematics",
    type: "UG Regular",
    duration: 4,
    description:
      "Focuses on pure theory, geometry, metrics, and complex algorithms.",
  },
  {
    deptCode: "COMP",
    code: "BCA",
    ...
    duration: 3,
    description:
      "Three-year applied training framework mapping software, MERN, and networks.",
  },
  {
    deptCode: "COMP",
    code: "MCA",
    ...
    duration: 2,
    description:
      "Two-year postgraduate tier evaluating advanced microservices and computing.",
  },
  {
    deptCode: "COMM",
    code: "BCOM-H",
    name: "B.Com (Honors) Accounting",
    type: "UG Regular",
    duration: 4,
    description:
      "Comprehensive management track focusing on calculations and audits.",
  },
];
...
async function main() {
  ...
  console.log(
    `🧹 Running script-level database seeding (Suffix: ${suffix})...`,
  );
  ...
  try {
    ...
    const insertedSessions = await db
      ...
      .values(sessionData)
      .returning({
        id: academicSessionTable.id,
        name: academicSessionTable.name,
      });
    ...
    const insertedDepts = await db
      ...
      .values(
        DEPARTMENTS.map((d) => ({
          ...d,
          code: `${d.code}-${suffix}`.slice(0, 10),
          name: `${d.name} ${suffix}`.slice(0, 30),
        })),
      )
      .returning({
    ...
    const courseInsertValues = COURSES_POOL.map((c) => {
      ...
      if (!dept) {
        throw new Error(
          `Department with code ${targetDeptCode} not found during seeding runtime.`,
        );
      }
      return {
    ...
    const finalCourseSessions = await db
      ...
      .returning({
        id: courseSessionTable.id,
        courseId: courseSessionTable.courseId,
        sessionId: courseSessionTable.sessionId,
      });
    ...
    for (const cs of finalCourseSessions) {
      ...
      const batchName = `${parentCourse.code} (${parentSession.name})`.slice(
        0,
        30,
      );
      ...
      for (let semNum = 1; semNum <= totalSemesters; semNum++) {
        const roman =
          ROMAN_NUMERALS[(semNum - 1) % ROMAN_NUMERALS.length] ||
          semNum.toString();
        ...
        const isTechOrScience = [
          "BCA",
          "MCA",
          "BSC-PHYS",
          "BSC-CHEM",
        ].includes(parentCourse.code);
        await db.insert(feeTable).values({
          semesterId: insertedSemester.id,
          institution: isTechOrScience
            ? faker.number.int({
                min: 18000,
                max: 26000,
              })
        ...
        const coursePrefix = parentCourse.code.includes("-")
          ? parentCourse.code.split("-")[1]
          : parentCourse.code;
        const prefixMatch = coursePrefix.slice(0, 3); // e.g., "PHY", "BCA", "CHE"

        // 1. Core Paper (MJC) -> Matches the stream prefix
        const corePaper = finalSubjects.find(
          (s) => s.type === "MJC" && s.code.startsWith(prefixMatch),
        );
        if (corePaper) {
          targetedSubjectIds.push(corePaper.id);
        }

        // 2. Subsidiary Paper (MIC) -> Separate elective domain
        const subsidiaryPaper = finalSubjects.find(
          (s) => s.type === "MIC" && !s.code.startsWith(prefixMatch),
        );
        if (subsidiaryPaper) {
          targetedSubjectIds.push(subsidiaryPaper.id);
        }

        // 3. Multi-Disciplinary (MDC) -> Elective course
        const mdcPaper = finalSubjects.find(
          (s) => s.type === "MDC" && !s.code.startsWith(prefixMatch),
        );
        if (mdcPaper) {
          targetedSubjectIds.push(mdcPaper.id);
        }

        // 4. Common Skill Competency / Values (SEC / VAC)
        const skillPaper = finalSubjects.find(
          (s) => s.type === "SEC" || s.type === "VAC",
        );
        if (skillPaper) {
          targetedSubjectIds.push(skillPaper.id);
        }
        ...
        if (semesterSubjectEntries.length > 0) {
          ...
          const uniqueEntries = Array.from(
            new Map(
              semesterSubjectEntries.map((item) => [
                item.subjectId,
                item,
              ]),
            ).values(),
    ...
    console.log(
      "🎉 Database successfully hydrated with fresh college data configuration!",
    );
  } catch (error) {
